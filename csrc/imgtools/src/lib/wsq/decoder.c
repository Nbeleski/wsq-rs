...
int wsq_decode_mem(unsigned char **odata, int *ow, int *oh, int *od, int *oppi,
  ...
  if ((ret = unquantize(&fdata, &dqt_table, q_tree, Q_TREELEN, qdata, width,
                        height))) {
    free(qdata);
    free_wsq_decoder_resources();
    return (ret);
  ...
  conv_img_2_uchar(cdata, fdata, width, height, frm_header_wsq.m_shift,
                   frm_header_wsq.r_scale);
...
int wsq_decode_file(unsigned char **odata, int *ow, int *oh, int *od, int *oppi,
  ...
  while (marker != SOF_WSQ) {
    if ((ret =
             read_table_wsq(marker, &dtt_table, &dqt_table, dht_table, infp))) {
      free_wsq_decoder_resources();
      return (ret);
    }
    if ((ret = read_marker_wsq(&marker, TBLS_N_SOF, infp))) {
...
/********************************************************************/
int huffman_decode_data_file(short *ip,            /* image pointer */
                             DTT_TABLE *dtt_table, /*transform table pointer */
                             DQT_TABLE *dqt_table, /* quantization table */
                             DHT_TABLE *dht_table, /* huffman table */
...
int decode_data_mem(
  ...
  for (inx = 1; (int)code > maxcode[inx]; inx++) {
    if ((ret =
             getc_nextbits_wsq(&tbits, marker, cbufptr, ebufptr, bit_count, 1)))
      return (ret);
    ...
    const int bits_req)     /* number of bits requested */
{
  ...
  if (bits_req <= *bit_count) {
    ...
    *bit_count = 0;
    if ((ret = nextbits_wsq(&tbits, (unsigned short *)NULL, file, bit_count,
                            bits_needed)))
      return (ret);
    bits |= tbits;
  }
